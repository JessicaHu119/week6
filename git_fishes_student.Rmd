...
[My GitHub Repository](https://github.com/JessicaHu119/week6.git)
...
# When we change 'echo = TURE' to 'echo = FALSE', the final document will not show the code. 'Echo = TURE' will show all the code, but 'echo = FALSE' will hide the code.
```
...
## A light red highlight indicates that this paragraph has been modified. A light red highlight overlapping in these places results in a dark red highlight indicating that the content inside has been removed, a green highlight indicates that something has been added, perhaps code or some text description, etc.
```
...
```{r, prettyfig, fig.align='center', fig.cap="add a legend here",eval = FALSE, echo=TRUE}
...
```{r, savefig, eval = FALSE, echo=TRUE}
...
```{r}
## The path to my image `output/figures/` appears in '.gitignore', while two png files appear in GitHub Desktop. I uploaded them and found a corresponding change in '.gitignore', but did not find the two png files in GitHub.
```
...
```{r, answer6}
## I want to keep track of the files in the pic/ folder on GitHub, even though they are .png files. Because I want other people, including my peers, to be able to find these files when they run my code on GitHub, otherwise they won't be able to get the drawing with the png image that I'm running.
```
...
```{r, table1, echo=TRUE, eval=FALSE, tab.cap = "difference in means, 95% confidence intervals, t-statistic, df and p-value"}
...
Mean activity for `r "Lemon"` in the control group was `r "test4$estimate[mean of x]"` (s / min) compared to the OA treatment group, which was `r "test4$estimate[mean of y]"` (s / min). The difference between control and OA treatment means was `r "abs(test4$estimate[mean of x]-test4$estimate[mean of y])
"` (s / min) (95% CI: `r "min(test4$conf.int)"` to `r "max(test4$conf.int)"`code for upper CI"`). 
...
The species I chose was `r "Lemon"`. The null hypothesis was that there was no significant difference in mean between the control and CO2 experimental groups for `r "Lemon"`. According to the above t. test about A, I calculated that its t-statistic is `r "test4$statistic"`, df is `r "test4$parameter"`, and p-value is `r "test4$p.value"`. The p-value is `r "test4$p.value"`, which is greater than 0.05, indicating that we have to accept our null hypothesis that in species `r "Lemon"`, there is no significant difference in mean between the experimental results of both the control and CO2 experimental groups. I also calculated the difference between the mean of the experimental and control groups, and it has an absolute value of `r "mean.difference4$mean"`.


Re-analyse the data for a single species using permutations instead of a t-test. Do your results differ?

```{r, Re-analyse, echo=TRUE, eval=FALSE}
...
```{r,permutations,echo=TRUE, eval=FALSE}
...
```
As shown in the results of the code above, we derived a p-value of 0.052 for the permutation test and a p-value of 0.054 for the t. test. our null hypothesis is that there is no significant difference in mean between the treatments. Because the difference in p-values is not significant, we accept the null hypothesis that there is no significant difference in mean between treatments.
